"""Optimise the CARRIER ORBIT itself, with PER-TARGET PRICES (stage 11 variant of carrier_opt.py).

Same 4-parameter pattern search, but the DP value counts a flyby of target x as w[x] instead of 1 (--weights a json
{target: price}).  Measured motive: over results/s7/libopt.pkl the carriers' natural cheap sets are heavily
concentrated -- 24 of the 298 targets are in NO carrier base, 74 in <= 2, while the most popular sits in 127 of 240.
That concentration, not route depth, is what stops the set cover: the most disjoint routes the pool can supply at
<= 0.040 J/flyby is FOUR.  Pricing the popular targets down moves the carrier GEOMETRY, which is the only lever that
changes which targets a route wants for free.
...
def evaluate(x):
    ...
    try:
        ...
        val, seq = CD.dp_route(t, lag, gap, ast, tL, a.lam, a.kgap, ac=ac_, w=g.get('w'))
    except Exception:
        return -1e9, 0, None
...
def main():
    ...
    ap.add_argument('--weights', default='', help='json {target: price}; missing targets get --wdef')
    ap.add_argument('--wdef', type=float, default=1.0)
    a = ap.parse_args()
    ...
    if a.weights:
        import json as _j
        pw = {int(k): float(v) for k, v in _j.load(open(a.weights)).items()}
        w = np.full(301, a.wdef); w[list(pw)] = [pw[k] for k in pw]
        print(f'weights: {len(pw)} priced, mean {np.mean(list(pw.values())):.3f}')
    _S.update(a=a, eph=eph, ids=ids, elem=elem, w=w)
