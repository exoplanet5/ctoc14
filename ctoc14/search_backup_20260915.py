...
def _expand_linear(eph, s, P, a):
    ...
    for j, tj in enumerate(tofs):
        ...
        f = np.isfinite(cost) & (cost <= P.dv_max)
        if not f.any():
            continue
...
def expand(eph, s, P):
    ...
    if P.tof_refine and len(ia):
        ...
        pl = np.where(np.isfinite(dv) & (dv <= 3.0 * min(P.dv_max, P.eta * a * tof.max())))[0]
        if len(pl):
            ...
            for step, half in ((1.0, 4.0), (0.25, 0.75)):
                ...
                tt_f = tt.ravel(); ia_f = np.repeat(ia[pl], nd); Rf = np.zeros((len(tt_f), 3))
                for q in np.where(okt.ravel())[0]:
                    Rf[q] = eph.ast_state(int(ia_f[q]), s.t + tt_f[q])[0]
    ...
    for o in keep:
        k = ia[o]
        children.append(State(t=s.t + tof[o], r=eph.ast_state(int(k), s.t + tof[o])[0], v=v2[o], m=s.m - dm[o], visited=s.visited | (1 << int(k)),
...
def roots(eph, t_launch_grid, m0, excluded, P, visited0=None):
    ...
    for tL in t_launch_grid:
        ...
        for tof in tofs:
            ...
            feas = np.isfinite(vinf) & (excess <= P.eta * a0 * tof) & (excess <= P.dv_max) & ~_visited_array(vis0) & (vinf <= P.vinf_cap)
